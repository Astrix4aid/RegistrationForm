...
const Reset=()=>{
  ...
  return (
    <>
    <div style={{width:'100%', minHeight:'100vh'}} className='d-flex justify-content-center align-items-center bg-secondary'>
      <div style={{width:'700px',backgroundColor:'white'}} className=' rounded p-5'>
        <h1 className='text-center'>Registration Form

        </h1>
        <p>
          <h5> <b>Name</b></h5>
          <TextField  value={name||''}  name='name' onChange={(e)=>InputValues(e.target)} style={{width:'600px'}} id="outlined-basic" label="Enter your name" variant="outlined" />  
                </p>
                 {/* Name Validation */}
            {InvalidName && 
              <div className=' my-3 text-danger  fw-bolder' >
                Invalid name...!!!
              </div>
            }
...
<Stack direction="row" className='d-flex justify-content-between' >
        <Button type='submit'  disabled={InvalidName || InvalidEmail || InvalidContact} onClick={Registration}   variant="contained" style={{width:'295px', color:'white'}} className='bg-info'>Register</Button>
